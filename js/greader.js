...
var THREE_COLUMN_VIEW, addFeed, auto_height, currentFeedUrl, debug_var, errorHandler, fs, generateContentList, generateFeed, generateFolder, generateOverview, getFavicon, getJsonFeed, importFromGoogleReader, importFromOpml, refreshFeed, removeAllFeeds, removeFeed, saveFavicon, showContent, showDetail, showFolderContent, showMenu, showSettingsPage, start_folder, subscriptions, toggle, toggleAddBox, toggleMenu, toggleStar, toggleThreeColumnView;
...
generateFolder = function(dict) {
  var folder, item, result, ul, _i, _len, _ref;
  ...
  folder.find("a:first").click(function() {
    return showFolderContent(dict);
  });
  return folder;
};
...
removeAllFeeds = function() {
  ...
  return localStorage.setItem("subscriptions", "[]");
};
  ...
  reader.onload = function(oFREvent) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (outline.attr("type") === "rss") {
        ...
        subscriptions.push(f);
        _results.push(localStorage.setItem("subscriptions", JSON.stringify(subscriptions)));
      }
    }
    return _results;
  };
...
toggleThreeColumnView = function() {
  if (THREE_COLUMN_VIEW === 0) {
    ...
    THREE_COLUMN_VIEW = 0;
    return $("#3-column-css").attr("disabled", "disabled");
  }
};
...
$(function() {
  var f, feed_ul, item, _i, _len, _results;
